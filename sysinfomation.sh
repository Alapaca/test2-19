...
eval $(df -T -m -x tmpfx -x devtmpfs | tail -n +2 | awk '{printf("paramount["NR"]=%d;parused["NR"]=%d;",$3,$5,$7,$6)} END {printf("parnum=%d\n",NR)}')

for (( i = 1; i <= $parnum; i++ )); do
    DiskSum=$[ $DiskSum + ${paramount[$i]} ]
    DisUsedSum=$[ $DisUsedSum + ${parused[$i]} ]
done

DiskUsedPercSum=$[( expr "$DiskUsedSum*100/$DiskSum" )]

DiskWarningLeve="normal"

if [[ $DiskUsedPercSum -gt 90 ]]; then
    DiskWarningLeve="warning"
elif [[ $DiskUsedPercSum -gt 80 ]]; then
    DiskWarningLeve="note"
fi
...
echo "$NowTime $HostName $OsType $KernelVersion $UpTime $LoadAvg $DiskSum ${DiskUsedPercSum}% $MemSum ${MemUsedPerc}% $CpuTemp $DiskWarningLeve $MemWarningLevel $CpuWarnLevel" 
